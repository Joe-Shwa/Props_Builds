...
1. **Add her as a test user.** The Google sign-in is still in *Testing* mode in Cloud Console, which means only pre-approved accounts can sign in.
   - Go to [Google Cloud Console](https://console.cloud.google.com/) → your project → **APIs & Services → OAuth consent screen → Audience/Test users**
   - Add her Google account email
   - This takes effect immediately, no waiting period

2. **Host the files.** Upload these three files to the same folder in your `joe-shwa.github.io` repo:
   - `prototype_tags.html`
   - `prototagicon192.png`
   - `prototagicon512.png`
...
3. **Have her create the Google Sheet** (see structure below), and a Drive folder for reference images. Share both links with yourself so you can sanity-check them, or just have her keep them handy for setup.
...
| Prototype Name | Category | Priority | Status | Description | Image | Fabricator | Submission Date | Completed Date |
|---|---|---|---|---|---|---|---|---|
...
- **Status:** Idea / In progress / Completed — used for her own filtering in the app's list view, but intentionally **left off the printed form**, since it's not relevant to the fabricator
...
- **Completed Date:** leave blank when submitting a prototype — the printed form shows a blank line for the fabricator to hand-write it in. If a date is already filled in on the sheet (e.g. after the fact), the form prints that value instead of a blank line.
...
1. Tap **Connect Google** → sign in → grant access (read Sheets, read Drive files, and a private settings space)
2. Paste in the **Sheet link** and the **image folder link**
3. Tap **Save & load prototypes**
